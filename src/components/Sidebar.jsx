...
import { Sparkles, History, User, PlusCircle, ArrowLeft } from 'lucide-react';

export const Sidebar = () => {
  const { activeScreen, navigate, user, handleLogout } = useApp();

  // If splash screen, don't show sidebar
  ...
  return (
    <div className="sidebar">
      ...
      {/* Center Stylized 'F' Emblem matching Reference Images */}
      ...
      {/* Optional Minimalist Bottom Nav Links */}
      {showNav && (
          ...
          {user && (
            <div style={{ marginTop: '8px', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {user.photoURL ? (
                  <img src={user.photoURL} alt={user.name} style={{ width: '24px', height: '24px', borderRadius: '50%' }} />
                ) : (
                  <User size={16} />
                )}
